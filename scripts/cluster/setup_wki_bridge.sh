...
#
# Usage:  sudo wos-setup-wki-bridge [N]
#
# Creates:
...
#
# To tear down:  sudo wos-setup-wki-bridge --teardown
# ----------------------------------------------------------------------------
...
tap_is_multiqueue() {
  ip tuntap show dev "$1" 2>/dev/null | grep -qw multi_queue
}
...
for ((i = 0; i < NUM_VMS; i++)); do
  ...
  if ip link show "$TAP" &>/dev/null; then
    if tap_is_multiqueue "$TAP"; then
      echo "  TAP ${TAP} already exists (multi_queue) - skipping"
      continue
    fi

    echo "  TAP ${TAP} already exists without multi_queue - recreating"
    ip link set "$TAP" down 2>/dev/null || true
    ip link set "$TAP" nomaster 2>/dev/null || true
    ip tuntap del dev "$TAP" mode tap
  fi

  echo "  Creating TAP: ${TAP} (owner: ${REAL_USER}, multi_queue)"
