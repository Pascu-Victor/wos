...
if [ "$HOST_SYSTEM" = "WOS" ]; then
    ...
    # These gnulib headers are generated by the first recursive build.  A
    # staged compiler cannot observe that later submitter-side mutation, so
    # materialize them before the configured tree is copied to the peers.
