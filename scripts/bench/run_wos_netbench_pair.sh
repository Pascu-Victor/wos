...
cleanup() {
  if [[ -n "$SERVER_PID" ]]; then
    ${REMOTE_SCRIPTS}/wos_ssh.sh "$SERVER_HOST" "kill $SERVER_PID >/dev/null 2>&1 || true; wait $SERVER_PID >/dev/null 2>&1 || true; rm -f $SERVER_LOG" >/dev/null 2>&1 || true
  fi
}
trap cleanup EXIT
...
SERVER_PEER="$(${REMOTE_SCRIPTS}/wos_resolve.py target "$SERVER_HOST")"
...
SERVER_PID="$(${REMOTE_SCRIPTS}/wos_ssh.sh "$SERVER_HOST" "rm -f $SERVER_LOG; $SERVER_REMOTE_CMD >$SERVER_LOG 2>&1 & echo \$!")"
...
RAW_RESULT="$(${REMOTE_SCRIPTS}/wos_ssh.sh "$CLIENT_HOST" "$CLIENT_REMOTE_CMD")"
RESULT="$(printf '%s\n' "$RAW_RESULT" | awk '/^\{.*\}$/ { line = $0 } END { print line }')"
SERVER_STDOUT="$(${REMOTE_SCRIPTS}/wos_ssh.sh "$SERVER_HOST" "cat $SERVER_LOG 2>/dev/null || true")"
