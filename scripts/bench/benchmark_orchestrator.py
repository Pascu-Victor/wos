...
def check_lan(router_ip: str, host_ip: str, ubuntu_hosts: list[str]) -> int:
    ...
    for host in ubuntu_hosts:
        reachable = False
        try:
            run_command([str(REMOTE_SCRIPTS / "linux_ssh.sh"), host, "true"])
            reachable = True
...
def run_suite(args: argparse.Namespace) -> int:
    ...
    if args.wos_fs_host and args.wos_fs_path:
        cmd = [
            str(BENCH_SCRIPTS / "run_wos_fsbench.sh"),
            args.wos_fs_host,
            "--mode",
            args.wos_fs_mode,
            "--path",
    ...
    if args.wos_net_server and args.wos_net_client:
        cmd = [
            str(BENCH_SCRIPTS / "run_wos_netbench_pair.sh"),
            args.wos_net_server,
            args.wos_net_client,
            "--mode",
            args.wos_net_mode,
...
def prepare_linux_cluster(args: argparse.Namespace) -> int:
    ...
    launch_cmd = [
        str(CLUSTER_SCRIPTS / "run_linux_cluster.sh"),
        str(args.num_vms),
        "--detach",
    ]
    if args.skip:
    ...
    if args.provision_hosts:
        ...
        provision_cmd = [
            str(BENCH_SCRIPTS / "provision_linux_cluster.sh"),
...
def stop_linux_cluster() -> int:
    result = run_command([str(CLUSTER_SCRIPTS / "run_linux_cluster.sh"), "--stop"])
    if result.stdout.strip():
...
def linux_cluster_status() -> int:
    result = subprocess.run(
        [str(CLUSTER_SCRIPTS / "run_linux_cluster.sh"), "--status"],
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
