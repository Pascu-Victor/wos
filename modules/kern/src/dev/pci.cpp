...
auto pci_find_ahci_controller() -> PCIDevice* {
    pci_log("pci: Scanning for AHCI controller...\n");

    // Test PCI access FIRST, before allocating
    pci_log("pci: Testing port I/O - reading vendor at slot 0:0...\n");
    uint32_t test_address = (0 << 16) | (0 << 11) | (0 << 8) | 0 | 0x80000000UL;
    pci_log("pci: Address to write to 0xCF8: 0x");
    pci_log_hex(test_address);
    pci_log("\n");

    outl(0xCF8, test_address);
    uint32_t test_data = inl(0xCFC);
    pci_log("pci: Data read from 0xCFC: 0x");
    pci_log_hex(test_data);
    pci_log("\n");

    if (test_data == 0xFFFFFFFF) {
        pci_log("pci: Port I/O appears broken - got 0xFFFFFFFF (no device)\n");
    } else if (test_data == 0) {
        pci_log("pci: Port I/O returned 0 - device might not exist or port access not working\n");
    } else {
        pci_log("pci: Port I/O returned valid-looking data\n");
    }
    ...
    for (uint8_t slot = 0; slot < 32 && device_count < MAX_PCI_DEVICES; ++slot) {
        for (uint8_t func = 0; func < 8; ++func) {
            ...
            pci_log("pci: Found device at slot ");
            pci_log_hex(slot);
            pci_log(":0x");
            pci_log_hex(func);
            pci_log(" - vendor 0x");
            pci_log_hex(vendor);
            pci_log(", device 0x");
            pci_log_hex(device_id);
            pci_log(", class 0x");
            pci_log_hex(class_code);
            pci_log(", subclass 0x");
            pci_log_hex(subclass);
            pci_log("\n");
            ...
            if (is_ahci) {
                pci_log("pci: FOUND AHCI SATA CONTROLLER at slot ");
                pci_log_hex(slot);
                pci_log(":0x");
                pci_log_hex(func);
                pci_log("!\n");
