...
auto wki_remote_net_attach(uint16_t owner_node, uint32_t resource_id,
                            const char* local_name) -> ker::net::NetDevice*;
...
void handle_net_op(const WkiHeader* hdr, uint16_t channel_id,
                    ker::net::NetDevice* net_dev, uint16_t op_id,
                    const uint8_t* data, uint16_t data_len);

// Consumer side: handle DEV_OP_RESP for NET proxy (SET_MAC, GET_STATS)
void handle_net_op_resp(const WkiHeader* hdr, const uint8_t* payload,
...
void handle_net_attach_ack(const WkiHeader* hdr, const uint8_t* payload,
                            uint16_t payload_len);
