...
void PageAllocator::init(uint64_t zone_base, uint64_t size_bytes) {
    ...
    uint64_t const META_BYTES = refcounts_offset + (static_cast<uint64_t>(total_pages) * sizeof(uint32_t));
...
void* PageAllocator::alloc(uint64_t size_bytes) {
