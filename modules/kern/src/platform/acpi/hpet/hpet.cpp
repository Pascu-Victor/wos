...
namespace ker::mod::hpet {
    ...
    void init() {
        ...
        mm::virt::mapPage(
            (mm::paging::PageTable*)mm::addr::getVirtPointer(rdcr3()),
            (uint64_t)hpetAddr,
            (uint64_t)mm::addr::getPhysPointer((uint64_t)hpetAddr),
    ...
    uint64_t usecToTicks(uint64_t us) {
        return (us * 1000000000) / tickPeriod;
    }

    uint64_t ticksToUsec(uint64_t ticks) {
        return (ticks * tickPeriod) / 1000000000;
    }

    uint64_t getUs() {
        return ticksToUsec(getTicks());
    }
