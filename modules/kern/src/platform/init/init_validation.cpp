...
static_assert(validation::validate_no_duplicates(MODULE_META_REGISTRY), "Duplicate module ID detected in kernel init registry");

// Validate all hard dependencies exist
static_assert(validation::validate_hard_deps_exist(MODULE_META_REGISTRY), "Missing hard dependency: a required module is not registered");
...
static_assert(!detect_cycle(build_adj_matrix(MODULE_META_REGISTRY)), "Circular dependency detected in kernel module initialization");
