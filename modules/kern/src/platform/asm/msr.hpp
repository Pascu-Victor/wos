...
static inline void cpuSetMSR(uint32_t msr, uint64_t value) {
    cpuSetMSR(msr, value & 0xFFFFFFFF, value >> 32);
}

static inline void cpuGetMSR(uint64_t msr, uint32_t *rax, uint32_t *rdx) {
    asm volatile("rdmsr"
                 : "=a"(*rax), "=d"(*rdx)
                 : "c"(msr));
}
