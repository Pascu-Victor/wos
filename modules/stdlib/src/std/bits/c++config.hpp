...
__attribute__((__always_inline__)) _GLIBCXX_CONSTEXPR inline bool __is_constant_evaluated() _GLIBCXX_NOEXCEPT {
...
extern "C++" _GLIBCXX_NORETURN void __glibcxx_assert_fail /* Called when a precondition violation is detected. */
    (const char* __file, int __line, const char* __function, const char* __condition) _GLIBCXX_NOEXCEPT;
#pragma GCC visibility pop
}  // namespace std
...
#define __glibcxx_assert(cond)                                                \
    do {                                                                      \
        if (__builtin_expect(!bool(cond), false)) _GLIBCXX_ASSERT_FAIL(cond); \
    } while (false)
#elif _GLIBCXX_HAVE_IS_CONSTANT_EVALUATED
// Only check assertions during constant evaluation.
namespace std {
__attribute__((__always_inline__, __visibility__("default"))) inline void __glibcxx_assert_fail() {}
}  // namespace std
#define __glibcxx_assert(cond)                                                      \
    do {                                                                            \
        if (std::__is_constant_evaluated())                                         \
            if (__builtin_expect(!bool(cond), false)) std::__glibcxx_assert_fail(); \
    } while (false)
#else
...
// implementation.
#if __cplusplus >= 201103L && _GLIBCXX_USE_CXX11_ABI && _GLIBCXX_USE_DUAL_ABI && __cpp_transactional_memory >= 201500L && \
    !_GLIBCXX_FULLY_DYNAMIC_STRING && _GLIBCXX_USE_WEAK_REF && _GLIBCXX_USE_ALLOCATOR_NEW
