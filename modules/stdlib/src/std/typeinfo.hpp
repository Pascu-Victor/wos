...
#include <std/bits/hash_bytes.hpp>
#endif
...
class type_info {
   public:
    ...
    bool operator!=(const type_info& __arg) const _GLIBCXX_NOEXCEPT { return !operator==(__arg); }
#endif
...
#if __GXX_TYPEINFO_EQUALITY_INLINE
inline bool type_info::before(const type_info& __arg) const _GLIBCXX_NOEXCEPT {
#if !__GXX_MERGED_TYPEINFO_NAMES
    // Even with the new abi, on systems that support dlopen
    // we can run into cases where type_info names aren't merged,
    // so we still need to do string comparison.
    if (__name[0] != '*' || __arg.__name[0] != '*') return __builtin_strcmp(__name, __arg.__name) < 0;
#else
...
_GLIBCXX23_CONSTEXPR inline bool type_info::operator==(const type_info& __arg) const _GLIBCXX_NOEXCEPT {
    if (std::__is_constant_evaluated()) return this == &__arg;
