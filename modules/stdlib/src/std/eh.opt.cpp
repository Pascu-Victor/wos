...
void _Unwind_Resume(struct _Unwind_Exception* exceptionObject) {
    __cxa_eh_globals* globals = __cxa_get_globals();
    globals->uncaughtExceptions++;
    exceptionObject->exception_cleanup(_URC_CONTINUE_UNWIND, exceptionObject);
}

extern "C" void __stack_chk_fail() {
    // Handle stack smashing detected
    hcf();
}
